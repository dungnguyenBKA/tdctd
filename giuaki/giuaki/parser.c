...
void compileAssignSt(void) {
  ...
  Type** listLeftType = malloc(100* sizeof(Type*));
  Type** listRightType = malloc(100* sizeof(Type*));
  if (listLeftType == NULL || listRightType == NULL) {
    printf("Error when calloc!");
    exit(1);
  }

  int Lvar = compileLeftAssign(listLeftType, 0);
  eat(SB_ASSIGN);
  int Rvar = compileRightAssign(listRightType, 0);
  ...
  for (int i = 0; i < Lvar; i++) {
    checkTypeEquality(listRightType[i], listLeftType[i]);
  }

  // free
  if (listLeftType != NULL) free(listLeftType);
  if (listRightType != NULL) free(listRightType);
}
/***/

int compileLeftAssign(Type** LAssign, int index) {
  LAssign[index] = compileLValue();

  switch (lookAhead->tokenType)
  {
  case SB_COMMA:
    eat(SB_COMMA);
    return compileLeftAssign(LAssign, index + 1);
    break;
  case SB_ASSIGN:
    return index + 1;
    break;
...
int compileRightAssign(Type** RAssign, int index) {
  RAssign[index] = compileExpression();

  switch (lookAhead->tokenType)
  {
  case SB_COMMA:
    eat(SB_COMMA);
    return compileRightAssign(RAssign, index + 1);
    break;
  default:
    return index + 1;
  }
