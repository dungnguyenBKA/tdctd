...
Token* readIdentKeyword(void) {
  ...
  while ((currentChar != EOF) && 
    ...
    if (count <= MAX_LENGTH) token->string[count++] = toupper((char)currentChar);
    readChar();
  }
    ...
    if (charCodes[currentChar] == CHAR_DIGIT) {
      ...
      if (charCodes[currentChar] == CHAR_DIGIT){
        ...
        if (charCodes[currentChar] == CHAR_RPAR){
          ...
          error(ERR_INVALIDDOUBLE, token->lineNo, token->colNo);
          return token;
        }
      }
      else {
...
Token* getToken(void) {
  ...
  /***/
  case CHAR_DIGIT: return readDoubleInt();
  /***/
